...
function TokenItem({
    ...
    return (
        <div
            ...
            style={style}
            onClick={() =>
                setToken(token.whitelisted ? token.symbol : token.id)
            }
        >
            <BasicTokenInfo
                token={token}
...
function ModalSelectToken({
    ...
    const renderFollowingTokenList = useMemo(() => {
        ...
        const res = followingTokenDataWithBalanceList.filter((token) => {
            ...
            if (token.symbol.toLowerCase().includes(str.toLowerCase()))
                return true;
            return false;
        });
        return res;
    ...
    const listHeight = useMemo(
        () => Math.min(680, Math.round(windowHeight / 2 / 68) * 68),
        [windowHeight],
    );
    const isSm = useIsSm();
