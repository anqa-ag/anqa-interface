...
export default function App() {
    ...
    const [debugData, setDebugData] = useState<{
        ...
        sources: "",
        feeRecipient:
            "0x2e8671ebdf16028d7de00229c26b551d8f145d541f96278eec54d9d775a49fe3",
        feeBps: 0,
    ...
    const setResetTimerFunc = (f: () => void) =>
        (resetTimerFunction.current = f);
    ...
    const [shouldUseDebounceAmountIn, setShouldUseDebounceAmountIn] =
        useState(true);
    const setTypedAmountIn = useCallback(
        (value: string, decimals = 8, shouldUseDebounce = true) => {
    ...
    const followingTokenData = useAppSelector(
        (state) => state.token.followingTokenData,
    );
    const tokenIn = useMemo(
        () =>
            (Object.values(followingTokenData) as Token[]).find((token) => {
                try {
                    const tokenSymbolOrAddress = location.pathname
                        .replace("/swap/", "")
                        .split("-")[0];
                    return (
                        token.symbol === tokenSymbolOrAddress ||
                        token.id === tokenSymbolOrAddress
                    );
                } catch {
                    return false;
                }
    ...
    const tokenInInfo: Token | undefined = useMemo(
        () => followingTokenData[tokenIn],
        [followingTokenData, tokenIn],
    );
    const tokenOutInfo: Token | undefined = useMemo(
        () => followingTokenData[tokenOut],
        [followingTokenData, tokenOut],
    );
    ...
    useEffect(() => {
        ...
        try {
            ...
            if (!tokenInSymbolOrAddress || !tokenOutSymbolOrAddress)
                throw new Error(`invalid pair = ${pair}`);

            const followingTokenDataList = Object.values(
                followingTokenData,
            ) as Token[];

            if (!fullTokenData || Object.values(fullTokenData).length === 0)
                return;
            const fullTokenDataList = Object.values(
                fullTokenData,
            ) as TokenInfo[];

            const newTokenIn =
                fullTokenDataList.find(
                    (token) => token.id === tokenInSymbolOrAddress,
                ) ||
                followingTokenDataList.find(
                    (token) => token.symbol === tokenInSymbolOrAddress,
                );
            const newTokenOut =
                fullTokenDataList.find(
                    (token) => token.id === tokenOutSymbolOrAddress,
                ) ||
                followingTokenDataList.find(
                    (token) => token.symbol === tokenOutSymbolOrAddress,
                );
            if (!newTokenIn)
                throw new Error(
                    `cannot find tokenIn = ${tokenInSymbolOrAddress}`,
                );
            if (!newTokenOut)
                throw new Error(
                    `cannot find tokenOut = ${tokenOutSymbolOrAddress}`,
                );
        ...
        (symbolOrAddress: string) => {
            ...
            try {
                ...
                if (!tokenInSymbolOrAddress || !tokenOutSymbolOrAddress)
                    throw new Error(`invalid pair = ${pair}`);
                navigate(
                    `/swap/${symbolOrAddress}-${tokenOutSymbolOrAddress}?${params.toString()}`,
                    { replace: true },
                );
            } catch (err) {
                pair !== "/swap" && console.error(err);
        ...
        (symbolOrAddress: string) => {
            ...
            try {
                ...
                if (!tokenInSymbolOrAddress || !tokenOutSymbolOrAddress)
                    throw new Error(`invalid pair = ${pair}`);
                navigate(
                    `/swap/${tokenInSymbolOrAddress}-${symbolOrAddress}?${params.toString()}`,
                    { replace: true },
                );
            } catch (err) {
                pair !== "/swap" && console.error(err);
    ...
    const followingPriceData = useAppSelector(
        (state) => state.price.followingPriceData,
    );
    const fractionalPriceTokenIn = useMemo(
        () =>
            followingPriceData[tokenIn]
                ? mulpowToFraction(followingPriceData[tokenIn])
                : undefined,
        [followingPriceData, tokenIn],
    );
    const fractionalPriceTokenOut = useMemo(
        () =>
            followingPriceData[tokenOut]
                ? mulpowToFraction(followingPriceData[tokenOut])
                : undefined,
        [followingPriceData, tokenOut],
    );
    ...
    const _fractionalAmountIn = useMemo(
        () =>
            typedAmountIn && tokenInDecimals !== undefined
                ? mulpowToFraction(
                      typedAmountIn.replaceAll(",", ""),
                      tokenInDecimals,
                  )
                : undefined,
        [tokenInDecimals, typedAmountIn],
    );
    const [fractionalAmountIn] = useDebounceValue(
        _fractionalAmountIn,
        shouldUseDebounceAmountIn ? 250 : 0,
    );
    ...
    const fractionalAmountOut = useMemo(
        () =>
            amountOut && tokenOutDecimals != undefined
                ? new Fraction(amountOut, Math.pow(10, tokenOutDecimals))
                : undefined,
    ...
    const readbleAmountOut =
        fractionalAmountOut && tokenOutDecimals !== undefined
            ? numberWithCommas(
                  truncateValue(
                      fractionalAmountOut.toFixed(18),
                      tokenOutDecimals,
                  ),
              )
            : "";
    ...
    const fractionalAmountInUsd = useMemo(
        () =>
            fractionalAmountIn && fractionalPriceTokenIn
                ? fractionalAmountIn.multiply(fractionalPriceTokenIn)
                : undefined,
        [fractionalAmountIn, fractionalPriceTokenIn],
    );
    ...
    const rate = useMemo(
        () =>
            fractionalAmountIn && fractionalAmountOut
                ? fractionalAmountOut.divide(fractionalAmountIn)
                : undefined,
        [fractionalAmountIn, fractionalAmountOut],
    );
        ...
        let res =
            fractionalAmountInUsd && fractionalAmountOutUsd
                ? fractionalAmountInUsd
                      .subtract(fractionalAmountOutUsd)
                      .divide(fractionalAmountInUsd)
                      .multiply(100)
                : undefined;
        if (res?.lessThan(0)) {
            res = new Fraction(0);
    ...
    const isPriceImpactVeryHigh = useMemo(
        () => Boolean(priceImpact?.greaterThan(10)),
        [priceImpact],
    );
    const isPriceImpactHigh = useMemo(
        () => Boolean(priceImpact?.greaterThan(5)),
        [priceImpact],
    );
    ...
    const isSufficientBalance =
        fractionalBalanceTokenIn && fractionalAmountIn
            ? fractionalBalanceTokenIn
                  .subtract(fractionalFeeAmount)
                  .equalTo(fractionalAmountIn) ||
              fractionalBalanceTokenIn
                  .subtract(fractionalFeeAmount)
                  .greaterThan(fractionalAmountIn)
                ? true
                : false
            : undefined;
    ...
    const onSetPercentAmountIn = (percent: number) => {
        if (fractionalBalanceTokenIn && fractionalFeeAmount) {
            let newTypedAmountIn = fractionalBalanceTokenIn
                .multiply(percent)
                .divide(100);
            if (
                fractionalBalanceTokenIn
                    .subtract(fractionalFeeAmount)
                    .lessThan(newTypedAmountIn)
            ) {
                newTypedAmountIn =
                    newTypedAmountIn.subtract(fractionalFeeAmount);
            }
    ...
    const swapButton = useMemo(() => {
        if (!fractionalAmountIn)
            return { isDisabled: true, text: "Enter an amount" };
        if (!isSufficientBalance)
            return { isDisabled: true, text: "Insufficient balance" };
        if (isValidatingQuote)
            return { isDisabled: true, text: "Getting quote..." };
        if (!fractionalAmountOut)
            return { isDisabled: true, text: "Not found route" };
    ...
    const [tokenInLogoSrc, setTokenInLogoSrc] = useState(
        tokenInInfo?.logoUrl || NOT_FOUND_TOKEN_LOGO_URL,
    );
    const [tokenOutLogoSrc, setTokenOutLogoSrc] = useState(
        tokenOutInfo?.logoUrl || NOT_FOUND_TOKEN_LOGO_URL,
    );
    useEffect(() => {
    ...
    const switchToken = useCallback(() => {
        ...
        try {
            ...
            if (!tokenInSymbolOrAddress || !tokenOutSymbolOrAddress)
                throw new Error(`invalid pair = ${pair}`);
            navigate(
                `/swap/${tokenOutSymbolOrAddress}-${tokenInSymbolOrAddress}?${params.toString()}`,
                { replace: true },
            );
        } catch (err) {
            pair !== "/swap" && console.error(err);
    ...
    const setTokenIn = useCallback(
        (symbolOrAddress: string) => {
            if (
                tokenOut === symbolOrAddress ||
                (tokenOutInfo && tokenOutInfo.symbol === symbolOrAddress)
            ) {
                switchToken();
            } else {
    ...
    const setTokenOut = useCallback(
        (symbolOrAddress: string) => {
            if (
                tokenIn === symbolOrAddress ||
                (tokenInInfo && tokenInInfo.symbol === symbolOrAddress)
            ) {
                switchToken();
            } else {
    ...
    const {
        txVersion: swapTxVersion,
        isSwapping,
        onSwap: _onSwap,
        success: isSwapSuccess,
    } = useSwap();
    const onSwap = () => {
        if (
            fractionalAmountIn &&
        ...
        <AppLayout>
            ...
            <div>
                    ...
                    <div>
                        {isDebug && (
                            <div className="absolute left-0 top-1/2 w-[250px] -translate-y-1/2 border-1 border-red-500 p-4">
                                <div>
                                    💡 Press cmd and click to multiple select
                                    source.
                                </div>
                                <a
                                ...
                                >
                                    tx_version:{" "}
                                    {swapTxVersion
                                        ? `https://aptoscan.com/transaction/${swapTxVersion}`
                                        : "--"}
                                </a>
                                <div>
                                    tx_success:{" "}
                                ...
                                <div>
                                    <select
                                        ...
                                        onChange={(e) =>
                                            setDebugData((prev) => ({
                                                ...prev,
                                                chargeFeeBy: e.currentTarget
                                                    .value as ChargeFeeBy,
                                            }))
                                        }
                                    >
                                        {["token_in", "token_out"].map(
                                            (option) => (
                                                <option key={option}>
                                                    {option}
                                                </option>
                            ...
                            <div className="mx-auto flex max-w-[464px] flex-col">
                                <div className="flex justify-end gap-2">
                                    <Button
                                        isIconOnly
                                        className={
                                            "h-[36px] w-[36px] min-w-min rounded border-1 border-black600 bg-black"
                                        }
                                        disableAnimation
                                        onPress={async () => {
                                            if (!isValidatingQuote)
                                    ...
                                    <Button
                                        ...
                                        className={
                                            ...
                                            " " +
                                            (isHighSlippage
                                                ? "bg-[rgba(255,153,1,0.2)]"
                                                : "bg-black")
                                        }
                                        onPress={() =>
                                            onOpenModal(MODAL_LIST.USER_SETTING)
                                        }
                                        disableAnimation
                                    >
                                        <BodyB2
                                                ...
                                                {account && (
                                                    ...
                                                    >
                                                        ...
                                                        <BodyB2 className="text-buttonSecondary">
                                                            {fractionalBalanceTokenIn
                                                                ? numberWithCommas(
                                                                      fractionalBalanceTokenIn.toSignificant(
                                                                          6,
                                                                      ),
                                                                  )
                                                                : "0"}
                                            ...
                                            <div className="flex items-center justify-between gap-3">
                                                <BodyB2 className="text-buttonSecondary">
                                                    {fractionalAmountInUsd
                                                        ? "~$" +
                                                          numberWithCommas(
                                                              fractionalAmountInUsd.toSignificant(
                                                                  6,
                                                              ),
                                                              false,
                                                              2,
                                                    ...
                                                    >
                                                        ...
                                                        <BodyB2 className="text-buttonSecondary">
                                                            {fractionalBalanceTokenOut
                                                                ? numberWithCommas(
                                                                      fractionalBalanceTokenOut.toSignificant(
                                                                          6,
                                                                      ),
                                                                  )
                                                                : "0"}
                                                ...
                                                <div className="flex items-center justify-between gap-1">
                                                    <BodyB2 className="text-buttonSecondary">
                                                        {fractionalAmountOutUsd
                                                            ? "~$" +
                                                              numberWithCommas(
                                                                  fractionalAmountOutUsd.toSignificant(
                                                                      6,
                                                                  ),
                                                                  false,
                                                                  2,
                                                ...
                                                priceImpact === undefined ? (
                                                    <div className="flex items-center gap-1">
                                                        <Icon
                                                            ...
                                                            color="#F44646"
                                                        />
                                                        <TitleT4 className="text-buttonRed">
                                                            Can&apos;t calculate
                                                            price impact
                                                        </TitleT4>
                                                    </div>
                                                    ...
                                                    <div className="flex items-center gap-1">
                                                        <Icon
                                                            ...
                                                            color="#FF9901"
                                                        />
                                                        <TitleT4 className="text-buttonYellow">
                                            ...
                                            <div className="flex items-center justify-between">
                                                ...
                                                {isValidatingQuote ? (
                                                    ...
                                                    <Button
                                                        ...
                                                        onPress={() =>
                                                            onOpenModal(
                                                                MODAL_LIST.TRADE_ROUTE,
                                                            )
                                                        }
                                                    >
                                                        <BodyB2>
                                                            {
                                                                sourceInfo.numberOfPaths
                                                            }{" "}
                                                            split
                                                            {sourceInfo.numberOfPaths >=
                                                            2
                ...
                */}
                ...
                <ModalSelectToken
                    isOpen={
                        globalModal === MODAL_LIST.SELECT_TOKEN_OUT &&
                        isModalOpen
                    }
                    onOpenChange={onOpenChangeModal}
                    onClose={onCloseModal}
                ...
                <ModalUserSetting
                    isOpen={
                        globalModal === MODAL_LIST.USER_SETTING && isModalOpen
                    }
                    onOpenChange={onOpenChangeModal}
                    onClose={onCloseModal}
                />
                <ModalTradeRoute
                    isOpen={
                        globalModal === MODAL_LIST.TRADE_ROUTE && isModalOpen
                    }
                    onOpenChange={onOpenChangeModal}
                    onClose={onCloseModal}
                    srcCoinType={tokenIn}
