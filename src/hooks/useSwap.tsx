...
export default function useSwap() {
    const [{ isSwapping, txVersion, success }, setSwapState] =
        useState<SwapState>({
            isSwapping: false,
            txVersion: undefined,
            success: undefined,
        });
    const { signAndSubmitTransaction, account, connected, isTelegram } =
        useAnqaWallet();
            ...
            try {
                ...
                if (
                    ...
                    (response.output === undefined ||
                        Object.keys(response.output).length === 0 ||
                ...
                const isUserRejectError =
                    jsonErr.toLowerCase().includes("user") &&
                    jsonErr.toLowerCase().includes("request");
