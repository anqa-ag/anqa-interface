...
const useTokenInfoFn = async ({
    ...
    const url =
        `${AGGREGATOR_URL}/v1/tokens?` +
        tokens.map((t) => `ids[]=${t}`).join("&");
...
function useWhitelistedTokens() {
    const fn = useCallback(async () => {
        const url =
            "https://raw.githubusercontent.com/anqa-ag/aptos-coin-list/main/anqaTokenList.json";
...
function FollowingTokenUpdater() {
    ...
    const followingTokenAddresses = useAppSelector(
        (state) => state.token.followingTokenAddresses,
    );
