...
const fn = async ({
    ...
    if (
        ...
        !tokenOut ||
        !amountInAfterFee ||
        parseFloat(amountInAfterFee) === 0
    )
        return;
    const excludeSources = ["bapt_swap_v1", "bapt_swap_v2", "bapt_swap_v2.1"];
    const response = await axios<GetRouteResponse>(
        `${AGGREGATOR_URL}/v1/quote`,
        {
            params: {
                srcCoinType: tokenIn,
    ...
    const amountInAfterFee = useMemo(
        () =>
            amountIn && feeBps && chargeFeeBy === "token_in"
                ? (
                      (BigInt(amountIn) * (BIP_BASE_BN - BigInt(feeBps))) /
                      BIP_BASE_BN
                  ).toString()
                : amountIn,
    ...
    const amountOutAfterFee = useMemo(() => {
        ...
        if (chargeFeeBy === "token_out") {
            const amountOutAfterFee = (
                (BigInt(amountOut) * (BIP_BASE_BN - BigInt(feeBps))) /
                BIP_BASE_BN
            ).toString();
