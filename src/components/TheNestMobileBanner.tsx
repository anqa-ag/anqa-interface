...
export default function TheNestMobileBanner() {
    ...
    const onTouchEnd = () => {
        ...
        if (isRightSwipe) {
            setIndex((prevState) =>
                prevState >= 1 ? prevState - 1 : items.length - 1,
            );
        }
    };
            ...
            <span className="absolute bottom-0.5 left-1/2 flex -translate-x-1/2 gap-1 ">
                {items.map((_, idx) => (
                    <button
                        key={idx}
                        className={
                            "h-1.5 w-1.5 rounded-full border-none bg-white outline-none " +
...
function SeasonBanner() {
    return (
        <div className="relative text-white">
            <Image
                width="100%"
                src={"/images/the_nest_mobile_banner_1.png"}
                className="static"
                radius="none"
            />
            <div className="absolute left-5 top-1/2 flex -translate-y-1/2 items-center">
                <TitleT1 className="mr-3 font-clashDisplayBold">
                    Season I
                </TitleT1>
            </div>
