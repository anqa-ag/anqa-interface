...
function getPetraWalletParams(
    ...
    const params = {
        ...
        dappEncryptionPublicKey: Buffer.from(
            bs58.decode(PETRA_ENCRYPTION_PUBLIC_KEY),
        ).toString("hex"),
...
export default function useTelegramWallet(): Omit<
    AnqaWalletState,
    "isTelegram"
> {
    const dispatch = useAppDispatch();

    const { address, publicKey, petraPublicEncryptedKey } = useAppSelector(
    ...
    const signAndSubmitTransaction = useCallback(
        ...
        async (payload: any) => {
            ...
            const params = getPetraWalletParams(
                payload,
                bs58.decode(petraPublicEncryptedKey),
            );
            TelegramWebApp.openLink(
                "https://petra.app/api/v1/signAndSubmit?data=" +
    ...
    const res = useMemo<Omit<AnqaWalletState, "isTelegram">>(
        () => ({
            ...
            wallet: account
                ? {
                      name: petraWallet.name,
                      icon: petraWallet.icon,
                      url: petraWallet.url,
                  }
                : null,
